...
require(GenomicSEM)
library(data.table)
source("function_rG_woSNP_CogentUKB.R")
...
for (i in 1:length(mungefile)) {
  results <- LDSCoutput_rG_woSNP("CP_Cogent.sumstats.gz","CP_UKB.sumstats.gz","EA.sumstats.gz", mungefile[i], sampleprev[i], populationprev[i], traitname[i])
