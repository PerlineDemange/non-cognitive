...
for (i in 1:length(mungefile)) {
  results <- CogNoncog_gen_correlation("C.sumstats.gz","NC.sumstats.gz", mungefile[i], sampleprev[i], populationprev[i], traitname[i])
  gencor_results <- rbind(gencor_results, results)  
}

write.csv(gencor_results, file=paste(outputName, '.csv', sep=''))